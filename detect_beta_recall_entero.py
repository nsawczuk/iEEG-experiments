...
def beta_bursts_3s(eeg, cdat, hdat, vdat,bp,k,length,block):
    #detects all bursts in a one second length clip
    if block==0:
    	times= np.linspace(0,int(length/1000), int(length/100), endpoint=False)[1:-1]
    elif block==28800:
    	times= np.linspace(0,int(length/1000), int(length/100), endpoint=False)[7:]
    else:
    	times= np.linspace(0,int(length/1000), int(length/100), endpoint=False)[2:-1]
    results=[]
    for t in times:
...
def beta_bursts(eeg,wavelet, freqoi, timeoi,tapM,k,length):
    ...
    for bloque in blocks:
        ...
        for i in range(transform_power.shape[0]):
            results= results+beta_bursts_3s(eeg, transform_power[i,:,:], timeoi, freqoi,i,k,length,bloque)
...
def detect_b(subject):
    ...
    DF.to_csv('beta/'+filename+"enterob.csv")
