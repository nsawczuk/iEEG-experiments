...
def detect_hg(subject):
  ...
  #  BIP= np.zeros((length*sessions,len(BIPOLAR)))
   # for i in range(len(BIPOLAR)):
    ...
    aux=[]
    for x in range(sessions*12):      
        sesion_number=random.randint(0, sessions-1)
        start= random.randint(0,length-3000)
        #eeg= BIP[:,sesion_number*length+start:sesion_number*length+start+3000]   
